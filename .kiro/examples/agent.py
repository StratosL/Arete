...
from tools import (
    ...
    list_available_tags_tool,
    get_documents_by_tag_tool,
)
...
async def retrieve_relevant_documents(
    ctx: RunContext[AgentDeps], user_query: str, tag: str = None
) -> str:
    """
    ...
    """
    print(f"Calling retrieve_relevant_documents tool with query: {user_query}, tag: {tag}")
    return await retrieve_relevant_documents_tool(
        ctx.deps.supabase, ctx.deps.embedding_client, user_query, tag=tag
    )
...
@agent.tool
async def list_available_tags(ctx: RunContext[AgentDeps]) -> List[str]:
    """
    Get all unique tags from the document knowledge base.

    Returns:
        List[str]: Sorted list of unique tags found across all documents
    """
    print("Calling list_available_tags tool")
    return await list_available_tags_tool(ctx.deps.supabase)


@agent.tool
async def get_documents_by_tag(ctx: RunContext[AgentDeps], tag: str) -> str:
    """
    ...
    Returns:
        str: Formatted string listing documents with the specified tag
    """
    print(f"Calling get_documents_by_tag tool with tag: {tag}")
    docs = await get_documents_by_tag_tool(ctx.deps.supabase, tag)

    if not docs:
        return f"No documents found with tag '{tag}'."

    # Format the results nicely
    formatted = f"Documents with tag '{tag}':\n\n"
    for doc in docs:
        formatted += f"• {doc['title']}\n"
        formatted += f"  ID: {doc['id']}\n"
        formatted += f"  URL: {doc['url']}\n"
        formatted += f"  Tags: {', '.join(doc['tags'])}\n\n"
